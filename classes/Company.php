...
class Company
{
    ...
    {
        // First check if any medicines are using this company
        $query = "SELECT COUNT(*) as count FROM medicines WHERE company_id = ?";
        $stmt = $this->conn->prepare($query);
        ...
        if ($row['count'] > 0) {
            return false; // Cannot delete company with associated medicines
        }
